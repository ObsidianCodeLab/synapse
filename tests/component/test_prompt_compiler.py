...
class TestBuildSystemPrompt:
    ...
    def test_agent_voice_empty_falls_back_to_synapse(self, tmp_path):
    ...
    def test_agent_voice_replaces_in_none_mode(self, tmp_path):
        ...
        from synapse.prompt.builder import PromptMode, build_system_prompt
        ...
        # "你是 Synapse" wording (other unrelated rules sections may still mention
        # the Synapse project by name; we only guard the identity self-reference).
        assert "你是 码哥，一个 AI 助手。" in prompt
        assert "你是 Synapse，一个 AI 助手。" not in prompt
...
class TestAgentResolveVoice:
    ...
    def test_resolve_voice_falls_back_to_settings_when_all_empty(self):
        ...
        agent._agent_profile = None
        agent.name = ""
        # settings.agent_name defaults to "Synapse"
