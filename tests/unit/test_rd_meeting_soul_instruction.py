...
def test_save_and_load_soul_instruction(work_home):
    payload = si.save_soul_instruction("关注账务中心限流模块与 REST 接入")
    assert payload["instruction"] == "关注账务中心限流模块与 REST 接入"
    assert payload.get("updated_at")

    path = work_home / "SOUL_INSTRUCTION.json"
    assert path.is_file()
    on_disk = json.loads(path.read_text(encoding="utf-8"))
    assert on_disk["instruction"] == payload["instruction"]

    assert si.load_soul_instruction() == payload["instruction"]
    assert si.save_soul_instruction_if_provided("") is False
    assert si.save_soul_instruction_if_provided("  有内容  ") is True


def test_format_soul_instruction_block_empty():
    assert si.format_soul_instruction_block() == ""
    assert si.format_soul_instruction_cli_lines() == []


def test_format_soul_instruction_block_renders(work_home):
    si.save_soul_instruction("优先查 MDB 配置")
    block = si.format_soul_instruction_block()
    assert "灵魂建议" in block
    assert "优先查 MDB 配置" in block
    assert "充分参考" in block

    cli_lines = si.format_soul_instruction_cli_lines()
...
def test_runtime_header_includes_soul_instruction(work_home, monkeypatch):
    si.save_soul_instruction("模块 A → 模块 B")
    ctx = make_context(
        role="host",
        binding={
            "node_id": "req_clarify",
