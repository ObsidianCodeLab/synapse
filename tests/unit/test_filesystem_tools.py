...
from synapse.tools.file import DEFAULT_IGNORE_DIRS, FileTool, GlobScanResult
...
def handler(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "synapse.core.policy_v2.get_config_v2",
        lambda: _Config(),
    )
    agent = MagicMock()
...
class TestGlob:
    ...
    def test_glob_scan_skips_heavy_runtime_dirs(self, file_tool, tmp_path):
        blocked = tmp_path / ".synapse" / "runtime" / "sessions"
        blocked.mkdir(parents=True)
        (blocked / "hidden.py").write_text("x", encoding="utf-8")
        workspace = tmp_path / ".synapse" / "workspaces" / "default"
    ...
    async def test_handler_glob_timeout_sets_cancel_flag(self, handler, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "synapse.tools.handlers.filesystem.settings.glob_timeout_sec",
            5,
            raising=False,
        )
        result = await handler.handle("glob", {"pattern": "*.py", "path": str(tmp_path)})
