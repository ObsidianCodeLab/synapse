...
from synapse.api.routes import bug_report
from synapse.api.routes import config as config_routes
from synapse.llm.capabilities import infer_capabilities, is_image_generation_model
...
def test_apply_llm_runtime_config_refreshes_all_runtime_components(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "synapse.llm.config.load_endpoints_config",
...
def test_collect_endpoint_summary_redacts_keys_and_keeps_diagnostic_fields(tmp_path, monkeypatch):
    ...
    import synapse.llm.config as llm_config
