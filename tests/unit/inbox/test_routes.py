...
from synapse.api.routes import inbox as inbox_routes
from synapse.inbox.models import InboxMessage
from synapse.inbox.service import InboxService
from synapse.inbox.store import InboxStore


async def test_inbox_routes_list_detail_and_mark(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr("synapse.config.settings.inbox_register_enabled", False)
