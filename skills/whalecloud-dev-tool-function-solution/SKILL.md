...
| `REPO_INFO` | 是 | 产品涉及的所有仓库信息列表，格式为：`应用模块`、`产品分支ID`、`产品分支`、`仓库地址`。示例：`- 应用模块：ZMDB - 产品分支ID: 4531 - 产品分支: CBOSS_BSS_ZMDB_V9.0_主分支 - 仓库地址: https://git-nj.iwhalecloud.com/xmjfbss/ZMDB.git`。从中抽取本次改造涉及的所有仓库，提取**应用模块**、**产品分支ID**和**仓库地址**（可能有多组）。 |
...
本技能产出体量大（`函数级方案.md` 常 > 1000 行、`func_solution_review.json` 含多张 Mermaid + 多条 plan），**必须**遵循：

- **禁止模型在单次工具调用里 inline 输出整篇大文档**（`函数级方案.md` 正文或完整 `func_solution_review.json`）——会触发输出 token 上限被截断，导致 `content` 丢失、`write_file` 报「缺少必要参数 content」。
- `函数级方案.md`：组装 `CONTEXT_JSON` 后交由 doc-generate 的 `scripts/fill_function_solution.py` 显式 UTF-8 **直接落盘**（见 Step 10），不内联整篇正文。
- `func_solution_review.json`：体量较大时，**先 `write_file` 落一个构建脚本 `{ARCHIVE_DIR}/.tmp/build_review.py`**（脚本内 `open(..., "w", encoding="utf-8")` + `json.dump(..., ensure_ascii=False)`，可按 overview / consistency_analysis / transformation_plans 分段拼装），再 `run_shell python {ARCHIVE_DIR}/.tmp/build_review.py` 生成；避免模型一次性吐出整段 JSON。
- **校验/统计一律走脚本**：任何对 `函数级方案.md` / `func_solution_review.json` 的行数统计、Mermaid 计数、JSON 结构校验，**必须** `write_file` 落 `*.py` + `run_shell python xxx.py`（脚本内 `open(encoding="utf-8")` 显式编码）。
  - **禁止** PowerShell（`Get-Content`/`Select-String`/`ConvertFrom-Json` 等）读写或校验含中文的文件（GBK/UTF-16 混乱会乱码，如 `MDB缁勪欢`）。
  - **禁止** `python -c "..."` 内联含中文或 f-string 的脚本（单行转义/括号极易 `SyntaxError`）。
- 完成后清理 `.tmp` 下临时脚本。
...
Step 0 — 参数校验与环境准备
  0a. 校验必填参数：WORK_ORDER_DIR, PRODUCT_CODE_ROOT, PRODUCT_DOC_ROOT, REPO_INFO
      ...
      - 解析 REPO_INFO：解析产品涉及的所有仓库信息，提取**应用模块**、**产品分支ID**和**仓库地址**（可能有多组），记为 REPO_DATA
      - 推导输出路径：
...
Step 2 — 解析上游输入物
  ...
  2b. 读取 MODULE_DOC（模块功能文档，路径由 WORK_ORDER_DIR 推导），提取：
      - 模块改造清单：修改模块 + 新增模块
      - 每条模块记录：模块名称 + 模块目标 + 满足的需求 + 验证方式 + 核心类 + 确认依据
      记为 MODULE_DATA
...
Step 3 — 数据设计
  ...
  3a. **识别数据需求**：
      - 分析模块目标与「满足的需求」，确定需要哪些数据结构（DTO/VO/实体类）
...
Step 10 — 文档落地（调用 whalecloud-dev-tool-doc-generate）
  10a. 复制 `references/function_solution_context.skeleton.json` 为 `ctx` 骨架，按「CONTEXT_JSON 字段契约」将 Step 1–9 映射填入（键名须与模板占位符**逐字**一致）
      ...
      - §1.3 `repos[]`：自 REPO_DATA 逐条映射 — `branch_id`←产品分支ID、`app_module`←应用模块（取展示名，如 `ZMDB`；若 REPO_INFO 为 `id|name` 则取 `|` 右侧）、`repo_url`←仓库地址、`change_desc`←本次改造涉及该仓库的摘要（无则 `"[待补充]"`）
      ...
      - **禁止**下列错误键名（`scripts/fill_function_solution.py` 会报错中止，勿与 `solution_review.json` 等其它技能混用）：
        ...
        - `repos[]`：`repo_name`/`repo_path`/`files`/`product_module_name`/`repo_module`→`branch_id`/`app_module`/`repo_url`/`change_desc`
...
Step 11 — 结构化评审 JSON（func_solution_review.json）
  ...
  11b. 写入 `{ARCHIVE_DIR}/func_solution_review.json`（UTF-8）：体量小可直接 `write_file`；含多张 Mermaid + 多条 plan 体量较大时，**按 A'**先 `write_file` 落 `{ARCHIVE_DIR}/.tmp/build_review.py`（`open(encoding="utf-8")` + `json.dump(ensure_ascii=False)`，分段拼装）再 `run_shell python` 生成，**禁止**模型一次性 inline 整段 JSON
  11c. 若存在人工评审意见（修订重跑）：**须优先理解** `human_review.comment` 与各 plan 的 `human_review.comment`，合理则调整方案后重新落盘双文件
  11d. 自检（**按 A' 一律走 `write_file` + `run_shell python xxx.py`，禁止 PowerShell / `python -c` 内联中文**）：plans 覆盖 MODULE_DOC 全部改造模块；无 consistency 悖论；JSON 可被前端评审面板解析（脚本内 `json.load(open(encoding="utf-8"))` 校验）
```
...
| 字段 | § | 对象键（每行） |
|------|---|----------------|
| `repos` | 1.3 | `branch_id`, `app_module`, `repo_url`, `change_desc` |
...
```json
{
  ...
  "PROD": "XXX系统",
  "STATUS": "draft",
  "repos": [{"branch_id": "4531", "app_module": "ZMDB", "repo_url": "https://git.example/ZMDB.git", "change_desc": "索引优先级改造"}],
  "terms": [],
  "data_structures": [],
  "db_changes": [],
  "message_contracts": [],
...
| 模型 inline 输出整篇 `函数级方案.md` / `func_solution_review.json` 致 `content` 截断 | **中止**，按 A' 改为脚本分段落盘（doc-generate 脚本 / `build_review.py`）后重试 |
| 用 PowerShell / `python -c` 内联读写或校验中文文件（乱码、`SyntaxError`） | **禁止**，改为 `write_file` 落 `*.py` + `run_shell python xxx.py`（显式 UTF-8）重试 |
