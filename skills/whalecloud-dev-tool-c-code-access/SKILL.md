...
Phase 2 — 入口精读（必读，本地读码）
  【Token 节约约束】
  - 读任何源文件前，**优先** Grep 定位目标符号/关键词的行号，再用 offset+limit 精确读取目标片段（±60 行）。
  - 确实需要了解文件全貌时（如理清整体结构、枚举所有分支），允许全文读取，但须在读前说明理由。
  - **严禁无目的分段滑窗**：禁止通过多次递增 offset（如 offset=0、300、600、900…）机械地扫完整个文件，而不知道自己在找什么。正确做法是先 Grep 找到所有目标符号的行号，按需读 2~3 个关键片段，无关部分跳过；若判断确实需要全文，直接一次性全文读取，不要分段凑。
