...
_BRAIN_SYSTEM_PROMPT = "You are assisting word-maker, an Synapse plugin for guided Word document generation."
...
class WordBrainHelper:
    ...
    def _brain_state(self) -> tuple[dict[str, Any], Any | None]:
        status: dict[str, Any] = {
            ...
            "message": "Synapse Brain is not available",
        }
        ...
        if not callable(get_brain):
            ...
            status["message"] = "Synapse host does not expose get_brain"
            return status, None
        try:
            brain = get_brain()
        except Exception as exc:  # noqa: BLE001 - status should explain host failures.
            status["reason"] = "host_brain_error"
            status["message"] = f"Synapse Brain lookup failed: {exc}"
            return status, None
        if brain is None:
            status["reason"] = "host_brain_unavailable"
            status["message"] = "Synapse Brain is not available"
            return status, None
        ...
        status.update(
            {
                ...
                "brain_injected": True,
                "reason": "available",
                "message": "Synapse Brain is available",
            }
        )
        return status, brain
