...
class InboxService:
    ...
    async def start(self) -> None:
        ...
        if self._task is not None and not self._task.done():
            return
        self._stop_event = asyncio.Event()
        self._task = asyncio.create_task(self._run_loop(), name="synapse-inbox-refresh")
    ...
    async def refresh(self) -> dict[str, Any]:
        ...
        if settings.inbox_broadcast_url:
            try:
        ...
        unified_messages: list[InboxMessage] = []
        if settings.inbox_unified_pull_enabled:
            try:
                from .unified_notify_client import UnifiedNotifyClient

                unified_messages = await UnifiedNotifyClient().fetch_system_notices()
            except Exception as exc:
                errors.append(f"unified: {exc}")
                logger.warning("[Inbox] unified service pull failed: %s", exc)

        messages = _dedupe_messages([*l0_messages, *l1_messages, *unified_messages])
        ...
        self._last_l1_count = len(l1_messages) + len(unified_messages)
    ...
    async def ingest_push(
        ...
        *,
        ...
        """Accept messages pushed by Synapse unified service (POST /api/inbox/push)."""
        if not settings.inbox_enabled:
            return {"ok": False, "stored": 0, "errors": ["inbox disabled"]}
        ...
        for raw in payloads:
            if not isinstance(raw, dict):
                errors.append("invalid payload: not an object")
                continue
            try:
                msg = InboxMessage.from_payload(raw, source=source)
                if not msg.id or not msg.title:
                    errors.append(f"invalid payload: missing id/title ({raw.get('id')!r})")
                    continue
                messages.append(msg)
            except Exception as exc:
        ...
        for message in after_messages:
            if message["id"] in after_ids - before_ids:
                _fire_event(
                    "inbox:new_message",
                    {
                        "id": message["id"],
        ...
        return {
            "ok": not errors or bool(messages),
            "stored": len(messages),
            "unread_count": after_unread,
            "errors": errors,
        }
