...
def sum_node_metrics_tokens(room_state: dict[str, Any], scope_id: str = "") -> int:
    """从 ``node_metrics`` 汇总整场会议 token（进行中节点优先 activity 动态值）。"""
    ...
    cur = str(room_state.get("current_node_id") or "").strip()
    status = str(room_state.get("status") or "")
    total = 0
...
def load_room_state(scope_id: str) -> dict[str, Any] | None:
    ...
    if not isinstance(metrics, dict):
        return data
    expected = sum_node_metrics_tokens(data, scope_id)
    stored = int(metrics.get("tokens") or 0)
    if expected != stored:
        metrics = dict(metrics)
        metrics["tokens"] = expected
        data = dict(data)
        data["metrics"] = metrics
        save_room_state(scope_id, data)
    return data


def save_room_state(scope_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    if isinstance(payload.get("node_metrics"), dict):
