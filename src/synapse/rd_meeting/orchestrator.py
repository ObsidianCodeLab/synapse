...
def _resolve_meeting_coordinator_loop() -> asyncio.AbstractEventLoop | None:
    """会议室 AI 节点 / 门控协程优先在 engine loop 上调度，避免占满 API loop。"""
    try:
        ...
        engine = None
    if engine is not None and not engine.is_closed():
        _remember_scheduler_loop(engine)
        return engine
    if _meeting_scheduler_loop is not None and not _meeting_scheduler_loop.is_closed():
        return _meeting_scheduler_loop
    try:
        current = asyncio.get_running_loop()
    except RuntimeError:
        return None
    _remember_scheduler_loop(current)
    return current


def _schedule_on_coordinator_loop(
    runner: Callable[[], Any],
    *,
    key: str = "",
    bucket: dict[str, asyncio.Task[None]] | None = None,
) -> None:
    """在协调 loop（优先 engine）上创建后台 task；可从 API / worker / engine 线程调用。"""
    coordinator_loop = _resolve_meeting_coordinator_loop()
    if coordinator_loop is None or coordinator_loop.is_closed():
        raise RuntimeError("no meeting coordinator event loop")
    ...
    def _start() -> None:
        if bucket is not None and key:
            ...
            if existing is not None and not existing.done():
                return
            bucket[key] = coordinator_loop.create_task(runner())
        else:
            coordinator_loop.create_task(runner())
    ...
    if current is coordinator_loop:
        _start()
    else:
        coordinator_loop.call_soon_threadsafe(_start)
...
def schedule_pipeline_background(
    ...
    *,
    ...
    try:
        _schedule_on_coordinator_loop(_runner, key=key, bucket=_pipeline_tasks)
    except RuntimeError:
        fn()
    return key
...
class MeetingRoomOrchestrator:
    ...
    def on_node_complete(
        ...
        *,
        ...
        bypass_downstream_gate: bool = False,
    ) -> dict[str, Any]:
        sid = scope_id.strip()
        ...
        if downstream_block_reason and requested_advance:
            ...
            room_state["intervention_kind"] = "result_confirm"
            ...
            if not pending.get("review_payload"):
                ...
                review_payload = load_node_review(sid, node_id)
    ...
    async def enter_unit_test_review_gate(
        self,
        *,
        scope_type: str,
        scope_id: str,
        ...
        """测试案例节点：加载 unit_test_review.json，进入专用人工评审面板。"""
        from synapse.rd_meeting.unit_test_gate import (
            clear_revision_context,
            ensure_human_review_pending_for_gate,
            load_unit_test_review_payload,
            validate_unit_test_review_json,
        )

        sid = scope_id.strip()
        set_phase(sid, "result_gate")
        ok, val_errors = validate_unit_test_review_json(sid)
        if not ok:
            gate = self.mark_human_gate(
                scope_type=scope_type,
                scope_id=sid,
                ...
                node_id=node_id,
                reason=(
                    f"{node_display_name(node_id)} 结构化测试案例产物未就绪："
                    + "; ".join(val_errors)
                ),
                ticket_title=ticket_title,
                hitl_form_schema=resolve_hitl_schema_for_gate(
        ...
        if isinstance(payload, dict):
            payload = ensure_human_review_pending_for_gate(sid, payload)
        clear_revision_context(sid)
        pending: dict[str, Any] = {
            "node_id": node_id,
            "report_body": report_body,
            ...
            "duration_seconds": duration_seconds,
            "stage_id": stage_id,
            "unit_test_review_payload": payload,
        }
        gate = self.mark_human_gate(
            scope_type=scope_type,
            scope_id=sid,
            room_id=room_id,
            node_id=node_id,
            reason=f"{node_display_name(node_id)} 待人工测试案例评审（执行测试 + 逐条确认）",
            ticket_title=ticket_title,
            hitl_form_schema=None,
            pending_delivery=pending,
        ...
        append_history_event(
            sid,
            {
                "event": "unit_test_review_gate",
                "room_id": room_id,
                "node_id": node_id,
                "text": f"{node_display_name(node_id)} 待人工测试案例评审",
                "intervention_kind": "unit_test_review",
                "tokens_used": tokens_used,
                "duration_seconds": duration_seconds,
                "log_type": "info",
    ...
    def confirm_unit_test_review_decision(
        self,
        *,
        scope_type: str,
        scope_id: str,
        ...
        """测试案例评审：全部用例确认且测试通过后推进；需修订则落盘意见并增量重跑。"""
        from synapse.rd_meeting.unit_test_gate import apply_human_decision
        ...
        if dec == "revise":
            from synapse.rd_meeting.unit_test_gate import write_revision_context

            write_revision_context(sid, payload, comment)
            append_history_event(
                sid,
                {
                    "event": "unit_test_needs_revision",
                    "room_id": room_id,
                    "node_id": node_id,
                    "comment": comment.strip(),
                    "log_type": "warning",
            ...
            detail = MeetingRoomService().resume_unit_test_revision(
                room_id,
                agent_pool=agent_pool,
            )
            return {
        ...
        rs.pop("pending_delivery", None)
        rs.pop("hitl_form_schema", None)
        rs.pop("unit_test_blocked", None)
        rs.pop("intervention_kind", None)
        save_room_state(sid, rs)
        append_history_event(
            sid,
            {
                "event": "unit_test_review_approved",
                "room_id": room_id,
                "node_id": node_id,
                "comment": comment.strip(),
                "log_type": "info",
    ...
    async def enter_node_review_gate(
        ...
        *,
        ...
        if uses_unit_test_gate(node_id):
            return await self.enter_unit_test_review_gate(
                scope_type=scope_type,
                scope_id=scope_id,
                room_id=room_id,
                node_id=node_id,
    ...
    def confirm_node_delivery(
        ...
        *,
        ...
        if not isinstance(pending, dict):
            raise ValueError("no_pending_delivery")
        has_report = bool(str(pending.get("report_body") or "").strip())
        has_review = isinstance(pending.get("review_payload"), dict) and bool(pending.get("review_payload"))
...
def schedule_run_node(
    *,
    ...
    try:
        _schedule_on_coordinator_loop(_runner, key=key, bucket=_running_tasks)
    except RuntimeError:
        logger.warning(
            "schedule_run_node: no event loop available, deferred pending_host_llm_begin_kind key=%s",
            key,
