...
//!   (~1-5 MB) dump into `~/.synapse/crashdumps/`.
...
//!   `\synapse-{pid}-{ticks}.dmp`, opens the file via
...
//! looks for `~/.synapse/crashdumps/*.dmp` regardless of platform, so
...
mod imp {
    ...
    /// Pre-built wide-char prefix `<dump_dir>\synapse-`. We append the
    ...
    pub fn install(dump_dir: &Path) {
        ...
        // need to touch formatting APIs. Layout: "<dump_dir>\synapse-".
        let prefix: Vec<u16> = OsStr::new(&format!("{}\\synapse-", dump_dir.display()))
            .encode_wide()
            .collect();
    ...
    unsafe extern "system" fn crash_filter(info: *const EXCEPTION_POINTERS) -> i32 {
        ...
        // Build "<dir>\synapse-<pid>-<tick>.dmp\0" in a fixed stack
