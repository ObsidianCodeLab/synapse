 ...
 *   1) bridge:render-ready  -- plugin called window.Synapse.ready() (best;
 ...
 *      for plugins that did NOT call Synapse.ready())
...
export default function PluginAppHost({ pluginId, apiBase, onViewChange }: PluginAppHostProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("synapse:plugin-reloaded", onPluginReloaded);
    return () => window.removeEventListener("synapse:plugin-reloaded", onPluginReloaded);
  }, [pluginId]);
  ...
  const handleIframeLoad = useCallback(() => {
    ...
    // handshake / Synapse.ready() will normally fire well within it; if
